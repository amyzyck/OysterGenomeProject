...
datchr <- alldat %>% filter(chrom == mychrom) %>% select("chromStart","chromEnd", "OutFLANK_0.2_bigsnpr_0.8.2_FST_unrelated", "OutFLANK_0.2_bigsnpr_0.8.2_pvaluesRightTail_unrelated", "OutFLANK_0.2_bigsnpr_0.8.2_OutlierFlag_unrelated")
datchr <- datchr %>% mutate(OutFLANK_0.2_bigsnpr_0.8.2log10_pvaluesRightTail_unrelated = -log10(OutFLANK_0.2_bigsnpr_0.8.2_pvaluesRightTail_unrelated))

y1_points <- datchr$OutFLANK_0.2_bigsnpr_0.8.2log10_pvaluesRightTail_unrelated > 5
dat_points <- datchr %>% filter(y1_points)
dat_bins <- datchr #%>% filter(!y1_points)
dim(dat_points)
head(dat_points)
...
# Plot FST colored by P-value
  p <- ggplot(dat_bins, aes(x = chromStart, y=OutFLANK_0.2_bigsnpr_0.8.2_FST_unrelated)) + geom_bin2d(binwidth = c(10000, 0.01), color="blue", alpha=0.5) + theme_classic() + 
  geom_point(data=dat_points, aes(x=chromStart, y=OutFLANK_0.2_bigsnpr_0.8.2_FST_unrelated), colour="darkblue", alpha = 0.5,  size=0.3)
  p
