...
if [ "$SNP" != "no" ]; then
	...
	FB2=$(( $NUMProc / 4 ))
	export FB2
	split -n l/$NUMProc --additional-suffix=.bed -d mapped.bed mapped. 
...
###New implementation of SNP calling here to save on memory	
	...
	ls mapped.*.bed | sed 's/mapped.//g' | sed 's/.bed//g' | shuf | parallel --bar --halt now,fail=1 --env call_genos --memfree $MAXMemory -j $NUMProc --no-notice "call_genos {} " 
